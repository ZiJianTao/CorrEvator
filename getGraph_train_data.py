...
#RQ1:COMPARE WITH BASELINE
group_id = int(sys.argv[1])
fileName = fileName + "_" + str(group_id)
window_size = 20

data_folder = 'DATA/GROUP'+str(group_id)+'/train-dataset/'
data_test_folder = 'DATA/RQ1/BATS0.8/'
dataset = [
...
[data_test_folder + 'preprocessed_BATS0.8_'+str(group_id)+'_test_1.txt', data_test_folder + 'preprocessed_BATS0.8_'+str(group_id)+'_test_1y.txt', "test"],
[data_test_folder + 'preprocessed_BATS0.8_'+str(group_id)+'_test_0.txt', data_test_folder + 'preprocessed_BATS0.8_'+str(group_id)+'_test_0y.txt', "test",],
]
'''
#RQ4:SIZE OF DATASET
# 定义参数和对应的百分比映射关系
...
# 根据输入参数获取相应的百分比
percentage_to_use = percent_mapping.get(percent, 1)  # 如果输入参数不在映射关系中，默认使用1
print("Percentage to use:", percentage_to_use)
window_size = 9
fileName = fileName + "_" + str(percent)


#RQ2:SIZE OF SLIDING WINDOW
#window_size = int(sys.argv[1])
#fileName = fileName + "_" + str(window_size)
'''
...
for s in dataset:
    print(f"Trying to load data from file: {s[0]}")
    X = localfile.try_get_file(s[0])
    print(f"Loading data from file: {s[0]}")

    y = localfile.try_get_file(s[1])

    if X is not None and y is not None:
        data_list.extend(X)
        print(f"Total data in data_list: {len(data_list)}")
        label_list.extend(y)
        if s[2] == "train":
        ...
        print(f"Failed to load data from files: {s[0]} or {s[1]}")


'''
RQ4
#随机化数据:训练集和测试集的数据被随机化。
train_ids = []
...
total_size = len(train_ids)  # 计算数据集大小
size_to_use = int(percentage_to_use * total_size)  # 计算要使用的数据集大小
# 切割数据集为所需大小
train_ids = train_ids[:size_to_use]
# 打印使用的数据集大小
print("train Data set size to use:", len(train_ids))
'''
...
'''
total_size = len(test_ids)  # 计算数据集大小
size_to_use = int(percentage_to_use * total_size)  # 计算要使用的数据集大小
# 切割数据集为所需大小
test_ids = test_ids[:size_to_use]
'''
# 打印使用的数据集大小
print("test Data set size to use:", len(test_ids))


#合并训练和测试数据:使用一个列表all_data_list来保存所有的数据
...
#为每个词汇分配一个ID:使用字典word_id_map将每个词汇映射到一个唯一的ID
word_id_map = {}
for i in range(vocab_size):
    word_id_map[vocab[i]] = i

#初始化词汇表外的词嵌入:对于词汇表中的每个词，如果它不在预训练的词嵌入中，则为其创建一个随机向量
...
def build_graph(start, end):
    ...
    for i in tqdm(range(start, end)):
     ...
     #   B_title_body_graph = add_other_features(B_title_graph, B_body_graph, all_data_list[i], 1, False)
        ...
        graph = link2graph(A_title_graph, B_title_graph)
        # print("graph", graph)
    ...
    return graphs
def build_graph_double(start1, end1, start2, end2):
    graphs = []
    for i in tqdm(range(start1, end1)):
        A_clean_title = None
        A_title_graph = None
        B_clean_title = None
        B_title_graph = None
    ...
    for i in tqdm(range(start2, end2)):
        A_clean_title = None
        A_title_graph = None
        B_clean_title = None
        B_title_graph = None
...
if __name__ == "__main__":
    ...
    print('building graphs for testing')
    test_graph = build_graph(start=train_size, end=train_size + test_size)

    #with open(data_folder + "%s.train_graphs" % fileName, 'wb') as f:
    #    pkl.dump(train_graph, f)
    #with open(data_folder + "%s.val_graphs" % fileName, 'wb') as f:
    #    pkl.dump(train_val_graph, f)
    with open(data_test_folder + "%s.test_graphs" % fileName, 'wb') as f:
