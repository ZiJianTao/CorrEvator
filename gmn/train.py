...
path='gmn/model_tzj_10_WS20/'

f_train = open(path+"train.txt","w",encoding="UTF-8")
f_valid = open(path+"valid.txt","w",encoding="UTF-8")
...
def batch_graph(data, fro, batch):
    ...
    print("Node Features Length:", len(node_fs))
...
def load_data(filename):
    names = ['train_graphs', 'val_graphs']
    objects = []
    for i in range(len(names)):
        with open("DATA/GROUP10/train-dataset/{}.{}".format(filename, names[i]), 'rb') as f:
            if sys.version_info > (3, 0):
                objects.append(pkl.load(f, encoding='latin1'))
            else:
                objects.append(pkl.load(f))
    # train_graphs,train_val_graphs,test_graphs = tuple(objects)
    train_graphs, train_val_graphs = tuple(objects)
    # return train_graphs, train_val_graphs, test_graphs
    return train_graphs, train_val_graphs
...
# train_graphs,train_val_graphs, test_graphs = load_data(fn)
train_graphs, train_val_graphs = load_data(fn)

#print("Train graphs:", train_graphs)
#print("Validation graphs:", train_val_graphs)
...
if os.path.exists(path + 'checkpoint'):
    saver.restore(sess, path + 'lyl.GMN-107')
else:
...
for i_iter in range(config["training"]["n_training_steps"]):
    ...
    #if i_iter % 1 == 0:
        #saver.save(sess,path + "%d/lyl.GMN" % (i_iter), global_step = 9)
    saver.save(sess, path + "%d/lyl.GMN" % (i_iter), global_step=9)
