...
def check_large(pull):
    ...
    path = '/DATA/lyl/pr_data/%s/%s/raw_diff.json' % (pull["base"]["repo"]["full_name"], pull["number"])
...
def fetch_pr_info(pull, must_in_local = False):
    ...
    path = '/DATA/lyl/pr_data/%s/%s' % (pull["base"]["repo"]["full_name"], pull["number"])
    parse_diff_path = path + '/parse_diff.json'
...
if __name__ == "__main__":
    ...
    #
    print(len(get_repo_info('kubernetes/kubernetes', 'pull', True)))
    # print(len(get_pull('saltstack/salt', 36357 , True)))